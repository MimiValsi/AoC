...
uint32_t part1(FILE *file);
uint32_t part2(FILE *file);
uint32_t nb_chars_code(char const *str);
uint32_t nb_chars_string(char const *str);
uint32_t new_chars_str(char const *str);

int main(void)
{
	FILE *file = OPENFILE;

	uint32_t p1 = part1(file);
	rewind(file);
	uint32_t p2 = part2(file);
	fclose(file);
	printf("part1: %d\n", p1);
	printf("part2: %d\n", p2);
	return 0;
...
uint32_t part1(FILE *file)
{
	/* ""           <- 2 chars of code  / 0 char of string
         ...
         * \x <- add 2 hex chars
         */
	uint32_t chars_code = 0;
	uint32_t chars_str = 0;
