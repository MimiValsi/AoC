...
struct Cities *init(char ida, char idb, char *city_a, char *city_b,
		    uint32_t dist);
struct Cities *append(struct Cities *c, char ida, char idb, char *city_a,
		      char *city_b, uint32_t dist);
...
uint32_t find_dist(struct Cities *c, char *visited);
...
int main()
{
	...
	uint32_t arr_len = strlen(visited);
...
void permute(struct Cities *c, char *a, int l, int r)
{
	static uint32_t min = 999;
	static uint32_t max = 0;
	if (l == r) {
		printf("%s\n", a);
		uint32_t sum = find_dist(c, a);
		if (max < sum) {
			max = sum;
		}
		if (min > sum) {
