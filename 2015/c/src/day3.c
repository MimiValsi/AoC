...
_Bool check_house(int64_t grid_x, int64_t grid_y, Position *pos, uint64_t size);
uint64_t input_1(FILE *file);
uint64_t input_2(FILE *file);
Grid move(char c, Grid *grid_1);

int main(void)
{
	FILE *file = fopen("data/day3.txt", "r");
	uint64_t unique_houses = input_1(file);
	uint64_t santa_and_robo_houses = input_2(file);
...
_Bool check_house(int64_t grid_x, int64_t grid_y, Position *pos, uint64_t count)
{
...
uint64_t input_1(FILE *file)
{
	uint64_t size = 5, unique_houses = 0;
...
uint64_t input_2(FILE *file)
{
	uint64_t size = 5, count = 0, unique_houses = 0;
	...
	for (uint64_t i = 0; i < end; i++) {
		...
		if (i >= size) {
			...
			Position *new_santa_pos =
				realloc(santa_pos, size * sizeof(*santa_pos));
			...
			Position *new_robo_pos =
				realloc(robo_pos, size * sizeof(*robo_pos));
		...
		santa_pos[i].presents = robo_pos[i].presents = pos[i].presents =
			0;
		...
		*grid = move(c, grid);
		...
		if (i % 2 == 0) {
			*santa_grid = move(c, santa_grid);
			...
			if (check_house(santa_pos[i].x, santa_pos[i].y, pos,
					count))
				unique_houses++;
			...
			*robo_grid = move(c, robo_grid);
			...
			if (check_house(robo_pos[i].x, robo_pos[i].y, pos,
					count))
				unique_houses++;
		}
	}
