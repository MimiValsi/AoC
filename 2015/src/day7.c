...
uint16_t part1(FILE *file);
void part2(FILE *file, uint16_t x);
struct Wire *init_wire(char const *id, uint16_t val);
struct Wire *append_wire(struct Wire *wire, char const *id, uint16_t val);
...
uint16_t find_source(struct Wire *wire, FILE *f);
void update_wire(struct Wire *wire, uint16_t val);
...
uint16_t part1(FILE *file)
{
	...
	uint16_t x = find_source(wire, file);
	...
	for (int i = 0;; i++) {
		...
		while (tmp) {
			...
			/*
                         ...
                         */
			if (isdigit(s.s[0][0]) &&
			    strcmp(s.s[2], tmp->id) == 0 &&
			    strcmp(s.s[1], "AND") == 0) {
				if (!source_exists(wire, s.s[4])) {
					wire = append_wire(wire, s.s[4],
							   atoi(s.s[0]) &
								   tmp->signal);
				}
			...
			/*
                         ...
                         * [0] [1] [2] [3]
                         */
			if (strcmp(s.s[0], "NOT") == 0 &&
			    strcmp(s.s[1], tmp->id) == 0) {
				if (!source_exists(wire, s.s[3])) {
					wire = append_wire(wire, s.s[3],
							   ~tmp->signal);
				}
			}
