...
void part1(FILE *file)
{
	...
	uint32_t first_coordinate = 0;

	uint32_t start_x = 0, end_x = 0, start_y = 0, end_y = 0;
	...
	for (uint32_t i = 0; i < NUM_LINES; i++) {
		...
		if (light_on == 1) {
			for (uint32_t j = start_x; j <= end_x; j++) {
				for (uint32_t o = start_y; o <= end_y; o++) {
					grid[j][o] = 1;
				}
			}
		}
